...
// minimum and maximum values for color gradient
const color_values = [0, 50, 100, 500, 1000, 0];
...
$(document).ready(function () {
  $.ajax({
    ...
    success: function (csvd) {
      ...
      color = d3.scale.linear().domain(color_values).range(color_gradient);

      // get first and last date
      max_date = new Date(covid_counties[0].date + " 00:00");
      min_date = new Date(
        covid_counties[covid_counties.length - 1].date + " 00:00"
      );
      ...
      slider = s_svg.append("g").attr("class", "slider").call(brush);

      slider.selectAll(".extent,.resize").remove();

      slider.select(".background").attr("height", s_height);
      ...
      d3.json("cb_2014_us_county_5m.json", function (json) {
        ...
        plot = svg
          ...
          .on("mouseover", function (d) {
            d3.select("#date").text(current_date.toDateString());
            d3.select("#county").text(d.properties.NAME);
            d3.select("#geoid").text(d.properties.GEOID);
            d3.select("#population").text(
              numberWithCommas(getPopulation(d.properties.NAME))
            );
...
function getCovidData(geoid, current_date) {
  ...
  let filtered = covid_counties.filter(function (i, n) {
    return (
      i.fips == geoid &&
      new Date(i.date + " 00:00").getMonth() == current_date.getMonth() &&
      new Date(i.date + " 00:00").getDate() == current_date.getDate()
    );
  });
  ...
  if (filtered.length > 0) {
    ...
    if (filtered.length > 1) {
      d3.select("#delta_confirmed").text(
        "(+" + (parseInt(filtered[0].cases) - parseInt(filtered[1].cases)) + ")"
      );
    }
  } else {
